...
// criar produto
// criar produto (versão sem id_categoria)
app.post('/produtos', async (req, res) => {
  const { nome, descricao, quantidade = 0, estoque_minimo = 0 } = req.body || {};

  // validação simples — só nome é obrigatório agora
  if (!nome) {
    return fail(res, 'Campo obrigatório: nome', 400);
  }
  ...
  try {
    const r = await pool.query(
      `INSERT INTO produtos (nome, descricao, quantidade, estoque_minimo)
       VALUES ($1, $2, $3, $4)
       RETURNING id_produto AS id, nome, descricao, quantidade, estoque_minimo`,
      [
        nome,
        descricao || null,
        Number(quantidade) || 0,
    ...
    // Em dev, deixa o console mostrar o erro completo pra diagnosticar rápido
    console.error('Erro ao inserir produto:', e.message || e);
    fail(res, e);
  }
});
...
app.put('/produtos/:id_produto', async (req, res) => {
  ...
  try {
    const r = await pool.query(
      ...
      [nome ?? null, quantidade ?? null, estoque_minimo ?? null, req.params.id_produto]
    );
    if (!r.rows.length) return fail(res, 'Produto não encontrado', 404);
    ok(res, r.rows[0]);
  } catch (e) { fail(res, e); }
  ...
  try {
    const r = await pool.query(
      `DELETE FROM produtos
        WHERE id_produto = $1
      RETURNING id_produto AS id, nome`,
      [req.params.id_produto]
    );
    ...
    ok(res, { mensagem: 'Produto excluído com sucesso', produto: r.rows[0] });
  } catch (e) {
    fail(res, e);
  }
});
...
app.post('/movimentacoes', async (req, res) => {
  const { id_produto, id_usuario, tipo, quantidade, data_movimentacao, observacao } = req.body || {};
  
  console.log("Dados recebidos no /movimentacoes:", { 
    id_produto, 
    id_usuario, 
    tipo, 
    quantidade, 
  ...
  if (!id_produto || !id_usuario || !tipo || !quantidade)
    return fail(res, 'Campos obrigatórios: id_produto, id_usuario, tipo, quantidade', 400);
  ...
  try {
    ...
    const up = await client.query(
      'UPDATE produtos SET quantidade = quantidade + $1 WHERE id_produto=$2 RETURNING id_produto, nome, quantidade, estoque_minimo',
      [delta, id_produto]
    );
    ...
    const ins = await client.query(
      `INSERT INTO movimentacoes (id_produto, id_usuario, tipo, quantidade, data_movimentacao, observacao)
       VALUES ($1,$2,$3,$4,COALESCE($5, NOW()),$6)
       RETURNING id, id_produto, id_usuario, tipo, quantidade, data_movimentacao, observacao`,
      [id_produto, id_usuario, String(tipo).toLowerCase(), Math.abs(quantidade), data_movimentacao || null, observacao || null]
    );
...
// histórico geral ou filtrado por produto (?id_produto=)
app.get('/movimentacoes', async (req, res) => {
  const { id_produto } = req.query;
  const hasFilter = !!id_produto;
const sql = `
  SELECT m.id, m.id_produto, p.nome AS produto_nome,
         m.id_usuario, u.nome AS responsavel_nome,
         m.tipo, m.quantidade, m.data_movimentacao, m.observacao
    FROM movimentacoes m
    JOIN produtos p ON p.id_produto = m.id_produto
    JOIN usuarios u ON u.id_usuario = m.id_usuario
   ${hasFilter ? 'WHERE m.id_produto = $1' : ''}
   ORDER BY m.data_movimentacao DESC, m.id DESC
`;
  try {
    const r = await pool.query(sql, hasFilter ? [id_produto] : []);
    ok(res, r.rows);
  } catch (e) { fail(res, e); }
});
