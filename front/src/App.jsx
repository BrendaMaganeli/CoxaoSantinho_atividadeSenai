...
export default function App() {
  ...
  // form produto
  const emptyProduto = { id_produto: null, nome: "", quantidade: 0, estoque_minimo: 0 };
...
const iniciarEdicao = (p) => {
  const produtoId = p.id_produto || p.id;
...
const excluirProduto = async (p) => {
   console.log("Produto para exclusão:", p); // DEBUG
  console.log("ID do produto:", p.id_produto, "ID:", p.id); // DEBUG
  if (!window.confirm(`Excluir o produto "${p.nome}"?`)) return;
  
  try {
    // Use o mesmo método que usamos na edição para pegar o ID
    const produtoId = p.id_produto || p.id;
    
    if (!produtoId) {
      alert("Erro: Não foi possível identificar o ID do produto");
      return;
    }
...
const enviarMovimentacao = async () => {
  console.log("movProdutoId:", movProdutoId); // DEBUG
  console.log("user:", user); // DEBUG
  console.log("produtosOrdenados:", produtosOrdenados); // DEBUG
  ...
  try {
    const payload = {
      id_produto: Number(movProdutoId),
      id_usuario: Number(user.id), // CORRIGIDO: usar id_usuario
      tipo: movTipo,
      quantidade: qtd,
      ...
      {view === "produtos" && (
        <section className="form" aria-label="produtos">
          ...
          <form onSubmit={buscar} style={{ width: "100%", display: "flex", gap: 8 }}>
            <input
              type="text"
              placeholder="Buscar por nome (ex.: abdutora)"
              value={q}
                  ...
                  {produtosOrdenados.map((p) => (
                    <tr key={p.id_produto}>
                      ...
                      <td style={{ display: "flex", gap: 6, justifyContent: "center" }}>
                        ...
                        <button type="button" onClick={() => excluirProduto(p)}>Excluir</button>
                      </td>
                    </tr>
                  ))}
        ...
        <section className="form" aria-label="estoque">
          ...
          <div style={{ width: "100%" }}>
            ...
            <ul style={{ listStyle: "none", paddingLeft: 0 }}>
              {produtosOrdenados.map((p) => (
                <li key={p.id_produto} style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <div style={{ width: "100%", marginTop: 10 }}>
            ...
            <div className="input-container">
              ...
              >
                ...
                {produtosOrdenados.map((p) => (
                  <option key={p.id_produto} value={p.id}>{p.nome}</option>
                ))}
              </select>
            </div>
